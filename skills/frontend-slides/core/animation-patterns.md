...
## GSAP Smooth Transitions (Optional)

Use when the user asks for smoother transition motion in plain HTML slides.
...
function animateVisibleSlide(slideEl) {
    if (reduceMotion || !window.gsap) return;

    gsap.fromTo(
        slideEl.querySelectorAll(".reveal"),
        { y: 24, opacity: 0 },
        {
            y: 0,
            opacity: 1,
...
For sequence-heavy hero slides, use `gsap.timeline().fromTo(...).to(...)`.
